...
export default function ColleagueBasicTable({
  rows,
  title,
  refetch,
  open,
    ...
    req.then((res) => {
      dispatch(
        ...
        dispatch(
          setEditId({
            editId: {
              catId: res.data.data.product.category._id,
              subCatId: res.data.data.product.subcategory._id,
            },
          })
        );
    });
  ...
  const HandelDelete = (event: any) => {
    ...
    isDeleting
      ? setTitleText(`برخورد کرده ایید  ${deletionError}   به مشکل`)
      : setTitleText(
          `اطمینان  دارید ؟ ${event.target.dataset.user}  ایا از حذف `
        );
            ...
            {rows?.map((row) => (
              ...
              >
                <TableCell align="center" component="th" scope="row">
                  <img
                    width={50}
                    src={`http://localhost:8000/images/products/images/${row.images?.[0]}`}
                  />
                </TableCell>
